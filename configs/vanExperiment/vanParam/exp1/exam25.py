...
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=[
            ...
            dict(type='DiceLoss', loss_name='loss_dice', ignore_index=0,  avg_non_ignore=True)]
    ),
    auxiliary_head=dict(
        in_channels=320,
        num_classes=4,
        loss_decode=[
            dict(type='CrossEntropyLoss', use_sigmoid=False, loss_name='loss_ce', class_weight=[1.0, 30.0, 30.0, 30.0],
                 loss_weight=1.0),
            dict(type='DiceLoss', loss_name='loss_dice', ignore_index=0, loss_weight=10.0,  avg_non_ignore=True)]
